...
module.exports = function registerEventStatus(api) {
  ...
  registerCommand({
    ...
    async execute(context) {
      ...
      const events = eventStore.listActiveEvents().filter((event) => event.guildId === message.guildId);
      if (events.length === 0) {
        await message.reply('Aktuell laufen keine Events.');
        return;
      }
      const embed = new EmbedBuilder().setTitle('Aktive Events').setColor(0x5865f2).setTimestamp(new Date());
