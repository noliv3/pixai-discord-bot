...
module.exports = function registerEventExport(api) {
  ...
  registerCommand({
    ...
    async execute(context) {
      ...
      const event = eventStore.getEvent(message.channelId);
      if (!event) {
      ...
      for (const upload of selected) {
        for (const attachment of upload.attachments) {
          try {
            ...
            if (!response.ok) {
              logger?.warn?.('Konnte Attachment nicht herunterladen', {
            ...
            logger?.error?.('Fehler beim Download für ZIP-Export', { error: error.message });
          }
        }
      }
